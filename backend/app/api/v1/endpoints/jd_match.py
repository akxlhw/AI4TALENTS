...
def get_llm_gateway():
    ...
    if not settings.LLM_ENABLED:
        raise HTTPException(
            status_code=503,
            detail="LLM features are disabled. Set LLM_ENABLED=true to enable."
        )
    gateway = create_llm_gateway()
    if not gateway:
        raise HTTPException(
            status_code=503,
            detail="Failed to create LLM gateway. Check LLM_API_KEY configuration."
        )
    return gateway
...
async def match_talents(
    ...
    **Match Score Components:**
    - Skill score (default weight: 0.4)
    - Research score (default weight: 0.3)
    - Experience score (default weight: 0.2)
    - Education score (default weight: 0.1)
