...
PRIVACY_POLICY_TEXT = """# 隐私政策（v2.1.0）
...
1. **严禁**通过邮箱、电话、社交媒体私信等任何渠道向人才本人发起招聘邀约、商业 solicitation 或任何未经人才本人同意的联系行为；
...
- **OpenAlex 学术数据库**：用于获取公开学术人才信息
- **GitHub API**：用于获取公开开源开发者信息
...
2.2 **禁止直接招聘联系**：您**承诺并保证**不会将平台展示的任何人才联系方式（包括但不限于电子邮箱、电话、社交媒体账号）用于：
- 直接向人才本人发起招聘邀约或职位推荐
...
2.5 **数据来源说明**：您理解并同意，平台展示的人才数据来源于公开学术数据库（OpenAlex）和开源社区（GitHub），平台仅提供数据聚合、检索与可视化服务，**不对数据的招聘可用性做任何授权或保证**。
...
3.2 人才数据的相关权利归数据原始发布方（OpenAlex、GitHub 等）所有，平台仅做合法范围内的引用与展示。
