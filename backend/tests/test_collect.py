...
class TestTechDomainCollectRepository:
    """技术领域采集配置Repository单元测试"""

    @pytest.fixture
    def repo(self, mock_session):
        return TechDomainCollectRepository(mock_session)
    ...
    async def test_list_with_collect_config(self, repo, mock_session):
        ...
        mock_domain.domain_name = "人工智能"
        mock_result.scalars.return_value.all.return_value = [mock_domain]
        mock_session.execute.return_value = mock_result
...
class TestDataFlowIntegration:
    ...
    async def test_std_to_serving_flow(
        ...
        assert saved_tag.tech_domain_id == test_data_setup["tech_domain"].tech_domain_id

        # 验证技术领域统计
        tech_repo = TechDomainRepository(test_session)
...
class TestEdgeCases:
    ...
    async def test_empty_collect_sources(
        ...
        """
        # 创建新的技术领域（没有Venue绑定）
        new_domain = TechDomain(
            domain_code="TEST-EMPTY",
            domain_name="测试空配置",
            is_enabled=True,
        )
        ...
        bindings = await binding_repo.get_by_tech_domain(new_domain.tech_domain_id)
...
class TestTalentQueryOptimization:
    ...
    async def test_update_topic_tags_filters_by_tech_domain(
        ...
        # 创建第二个技术领域
        tech_domain_2 = TechDomain(
            domain_code="CV",
            domain_name="计算机视觉",
            is_enabled=True,
            sort_order=2,
        )
        test_session.add(tech_domain_2)
        await test_session.flush()

        # 创建两个任务，关联不同的技术领域
        task_1 = CollectTask(
            task_code="CR02-TEST-001",
            tech_domain_id=tech_domain_1.tech_domain_id,
            collect_mode="full",
            triggered_by=None,
        ...
        task_2 = CollectTask(
            ...
            tech_domain_id=tech_domain_2.tech_domain_id,
            collect_mode="full",
            triggered_by=None,
            triggered_at=datetime.utcnow(),  # 添加必填字段
            status="pending",
        ...
        # talent_2 不应该有 topic_tags 更新（关联 tech_domain_2，不在任务 1 范围内）
        # 注意：由于我们只更新了 task_1 相关的人才，talent_2 不应被修改
        # talent_2 没有关联到 task_1 的 tech_domain，所以不在更新范围内
