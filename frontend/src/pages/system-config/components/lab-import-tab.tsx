...
const LabImportTab: React.FC = () => {
  ...
  const beforeUpload: UploadProps['beforeUpload'] = (file) => {
    // Accept .jsonl (and plain text variants); reject others.
    const isJsonl =
    ...
    return false // prevent auto-upload; we handle it on button click
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setReport(res.data)
      message.success(
        `导入完成：${res.data.inserted} 人入库，${res.data.skipped} 行跳过`
      )
    } catch (e) {
        ...
        <Card title="导入报告">
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="跳过">
              <Tag color={report.skipped > 0 ? 'orange' : 'default'}>{report.skipped}</Tag>
            </Descriptions.Item>
          </Descriptions>
