...
export const openSourceApi = {
  ...
  getDeveloper: (id: number) => apiClient.get(`/open-source/developers/${id}`),
  exportDevelopers: (developerIds: number[], format: 'csv' | 'xlsx' = 'csv') =>
    apiClient.post(
      `/open-source/developers/export`,
      { developer_ids: developerIds, format },
      {
        responseType: 'blob',
      }
    ),
  ...
  getContributions: (id: number) => apiClient.get(`/open-source/developers/${id}/contributions`),
  getLanguages: (id: number) => apiClient.get(`/open-source/developers/${id}/languages`),
  ...
  search: (params: OSSearchParams) => apiClient.post('/open-source/search', params),
  ...
  getFavoriteIds: () => apiClient.get('/open-source/favourites/ids'),

  listTalentPools: () => apiClient.get('/open-source/talent-pools'),
  ...
  getDiscoveryStatus: () => apiClient.get('/open-source/discover/status'),
  importDiscovered: (repos: { repo_full_name: string; tech_element: string[] }[]) =>
    apiClient.post('/open-source/discover/import', { repos }),
  updateRepoConfig: (id: number, data: Record<string, unknown>) =>
    apiClient.put(`/open-source/repo-configs/${id}`, data),
  deleteRepoConfig: (id: number) => apiClient.delete(`/open-source/repo-configs/${id}`),
  ...
  collectRepo: (id: number, contributorsPerRepo?: number) =>
    apiClient.post(`/open-source/repo-configs/${id}/collect`, null, {
      params: { contributors_per_repo: contributorsPerRepo },
    }),
  ...
  getCollectTask: (id: number) => apiClient.get(`/open-source/collect/tasks/${id}`),
  createCollectTask: (data: Record<string, unknown>) =>
    apiClient.post('/open-source/collect/tasks', data),
  cancelCollectTask: (id: number) => apiClient.post(`/open-source/collect/tasks/${id}/cancel`),
  deleteCollectTask: (id: number) => apiClient.delete(`/open-source/collect/tasks/${id}`),

  jdMatch: (data: { jd_text: string; filters?: Record<string, unknown>; top_k?: number }) =>
    apiClient.post('/open-source/jd-match', data),

  getEmbeddingStatus: () => apiClient.get('/open-source/embeddings/status'),
  getEmbeddingProgress: () => apiClient.get('/open-source/embeddings/progress'),
  ...
  cancelEmbeddingGeneration: () => apiClient.post('/open-source/embeddings/cancel'),
}
